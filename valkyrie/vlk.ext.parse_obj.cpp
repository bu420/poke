...
image load_image(const std::string& path) {
    ...
    i32 height;
    i32 channels;
    u8* image_data = stbi_load(path.c_str(),
                               &width,
                               &height,
                               &channels, 0);
    ...
    return image{
        ...
        .data{
            std::vector<std::byte>(reinterpret_cast<std::byte*>(image_data),
    ...
    */
    ...
    while (current_newmtl_pos != tokens.end()) {
        ...
        if (material_pos != materials.end()) {
            throw std::runtime_error(
                std::format("Valkyrie: error when parsing mtl {}: material {} is already defined.", path, material_name));
        }
                      ...
                      [&](const auto& line_tokens) {
                          ...
                          if (index_pos != indices.end()) {
                              const std::string image_path =
                                  (std::filesystem::path{path}.remove_filename() / line_tokens.at(1)).string();
...
model vlk::parse_obj(const std::string& path) {
    ...
    while (mtllib_pos != tokens.end()) {
        const std::string mtl_path =
            (std::filesystem::path{path}.remove_filename() / mtllib_pos->at(1)).string();
    ...
    while (current_usemtl_pos != tokens.end()) {
        ...
        if (material_pos == materials.end()) {
            throw std::runtime_error(
                std::format("Valkyrie: error when parsing mesh {}: material {} does not exist.", path, material_name));
        }
